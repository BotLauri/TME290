...
int32_t main(int32_t argc, char **argv) {
  ...
  if (0 == commandlineArguments.count("cid")) {
    ...
    myfile << "time, battery, grassMean,\n";

    auto onSensors{[&od4, &myfile](cluon::data::Envelope &&envelope)
      {
        ...
        // The data.  
        myfile << msg.time() << "," << msg.battery() << ",";

        control.command(command);
        od4.send(control);
      }};

    auto onStatus{[&verbose, &myfile](cluon::data::Envelope &&envelope)
      {
        auto msg = cluon::extractMessage<tme290::grass::Status>(
            std::move(envelope));

        // The data.  
        myfile << msg.grassMean() << ",\n";
